...
int main(int argc, char *argv[])
{
    ...
    //     cpu_profiler.start();
        int iter_count=0;
        int dist_count=0;
        auto hnsw_result = alg_hnsw->searchKnnUntilAllGroundTruthFound((void*)(vecs[row].data()), cur_k,gtset,hop_id_lists[row],iter_count,dist_count);
    //     cpu_profiler.stop();
    ...
    save_hop_id_lists(hop_id_lists, out_dir + "/HNSWIO_HopIds.json", query_cnt);
    vecs.clear();
