...
int main(int argc, char *argv[])
{
    ...
    //     cpu_profiler.start();
        int iter_count=0;
        int dist_count=0;
        auto hnsw_result = alg_hnsw->searchKnnAdaptive((void*)(vecs[row].data()), cur_k,pc.dataset.hop_diff_limit,pc.dataset.stable_hops,pc.dataset.break_percent,dist_count,iter_count);
    //     cpu_profiler.stop();
    ...
    // 输出 JSON / CSV
    std::string out_dir = "/home/zqf/Hulu-Retriever/HopStateAdaptiveResults/" + dataset_name + "/" + std::to_string(num_threads)+"_"
