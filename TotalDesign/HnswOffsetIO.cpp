...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::vector<double>> iter_infos(6, std::vector<double>(pc.query_cnt, 0));
    auto block_res = get_block_size(pc.dev_path, pc.iovec_ext_number);
    ...
    //     cpu_profiler.start();
        int iter_count=0;
        int dist_count=0;
        int start_prefetch_iter=0;
        int last_prefetch_iter=0;
        int io_cnt=0;
        int query_type=0;
        auto hnsw_result = alg_hnsw->searchKnnEarlyOverlap((void*)(vecs[row].data()), cur_k,
            pc.dataset.hop_diff_limit,pc.dataset.stable_hops,pc.dataset.start_percent,pc.dataset.break_percent,
            pc.dataset.stableiter,pc.dataset.least_else_size,
            iter_count,dist_count,start_prefetch_iter,last_prefetch_iter,io_cnt,query_type);
    //     cpu_profiler.stop();
        ...
        iter_infos[0][row]=iter_count;
        iter_infos[1][row]=dist_count;
        iter_infos[2][row]=start_prefetch_iter;
        iter_infos[3][row]=last_prefetch_iter;
        iter_infos[4][row]=io_cnt;
        iter_infos[5][row]=query_type;
    ...
    generate_json_multi_T<double>(iter_infos, {"iter_count","dist_count","start_prefetch","last_prefetch","io_cnt","query_type"}, query_cnt, out_dir + "/prefetch.json");
