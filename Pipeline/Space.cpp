...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::vector<double>> prfetch_iters(5, std::vector<double>(pc.query_cnt, 0));
    auto block_res = get_block_size(pc.dev_path, pc.iovec_ext_number);
    ...
    //     cpu_profiler.start();
        ...
        int io_cnt=0;
        alg_hnsw->searchKnnPrefetchSpace((void*)(vecs[row].data()), cur_k,pc.dataset.stable_iter,pc.dataset.least_else_size,
    ...
    //     cpu_profiler.stop();
        ...
        prfetch_iters[0][row]=iter_count;
        prfetch_iters[1][row]=dist_count;
        prfetch_iters[2][row]=start_prefetch_iter;
        prfetch_iters[3][row]=last_prefetch_iter;
        prfetch_iters[4][row]=io_cnt;
    ...
    // 输出 JSON / CSV
    std::string out_dir = "./prefetch_results/"+pc.raw_or_basefs+"_results/Offset_results/" + dataset_name + "/" + std::to_string(num_threads)+"_"
    ...
    generate_json_multi_T<double>(prfetch_iters, {"iter_count","dist_count","start_prefetch","last_prefetch","io_cnt"}, query_cnt, out_dir + "/prefetch.json");
