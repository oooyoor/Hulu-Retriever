...
int main(int argc, char *argv[])
{
    ...
    //     cpu_profiler.start();
        // int iter_count=0;
        // int dist_count=0;
        alg_hnsw->searchKnnPrefetchBaseline((void*)(vecs[row].data()), cur_k,pc.dataset.stable_iter,pc.dataset.least_else_size,ioers[threadId]);
    //     cpu_profiler.stop();
        ...
        ioers[threadId]->drain_all_completions();
    ...
    // 输出 JSON / CSV
    std::string out_dir = "/home/zqf/Hulu-Retriever/ATimeResults/overlap_results/" + dataset_name + "/" + std::to_string(num_threads)+"_"
    ...
    generate_json_multi_T<double>(costs, {"hnsw","io","hnswio"}, query_cnt, out_dir + "/overlap.json");
