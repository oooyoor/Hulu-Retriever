...
int main(int argc, char *argv[])
{
    ...
    std::string iter_dist_count_dir_path="/home/zqf/Hulu-Retriever/SearchDifficultyResults/BaselineResults/";
    // std::string iter_json_path = iter_dist_count_dir_path+dataset_name+"/" + std::to_string(num_threads)+"_"
    //     +std::to_string(pc.dataset.search_ef)+"_"+std::to_string(pc.io_depths)+"/"+std::to_string(query_cnt)+"/"
    //     +std::to_string(repeat_id)+"/HNSWIO_IterDistCount.json";
    std::string iter_json_path = iter_dist_count_dir_path+dataset_name+"/32_"
        +std::to_string(pc.dataset.search_ef)+"_"+std::to_string(pc.io_depths)+"/"+std::to_string(query_cnt)
        +"/1/HNSWIO_IterDistCount.json";
    // 定义读取 json 文件的辅助函数
    auto read_json_file = [](const std::string& file_path) -> nlohmann::json {
    ...
    nlohmann::json iter_dist_json;
    try {
        iter_dist_json = read_json_file(iter_json_path);
    } catch (const std::exception& e) {
        cfg.logger->error(fmt::format("Failed to read json: {}", e.what()));
    }
    ...
    //     cpu_profiler.start();
        int iter_count = iter_dist_json[row]["iter_count"];
        auto hnsw_result = alg_hnsw->searchKnnIdealTime((void*)(vecs[row].data()), cur_k,iter_count);
    //     cpu_profiler.stop();
    ...
    // 输出 JSON / CSV
    std::string out_dir = "./ideal_time_results/" + dataset_name + "/" + std::to_string(num_threads)+"_"
