...
int main(int argc, char *argv[])
{
    ...
    //     CPUProfiler cpu_profiler;
        ...
        int iter_count=0;
    //     cpu_profiler.start();
        auto hnsw_result = alg_hnsw->searchKnnCnt((void*)(vecs[row].data()), cur_k,dist_count,iter_count);
    //     cpu_profiler.stop();
    ...
    // 输出 JSON / CSV
    std::string out_dir = "./recall_results/"+pc.raw_or_basefs+"_results/Offset_results/" + dataset_name + "/" + std::to_string(num_threads)+"_"
    ...
    cfg.logger->info("hnswio iter_dist_counts json path: {}", out_dir + "/HNSWIO_IterDistCount.json");
